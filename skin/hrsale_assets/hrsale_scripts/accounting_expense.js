$(document).ready(function () {
	...
	$("#xin-form").validate({
		rules: {
			...
			payee_id: { required: true },
			payment_method: { required: true },
			deposit_reference: { number: true },
		},
		// Specify validation error messages
		messages: {
			bank_cash_id: { required: "Please select account type" },
			amount: { required: "Please enter amount" },
			expense_date: { required: "Please enter deposit date" },
			company: { required: "Please select category" },
			category_id: { required: "Please select category" },
			payee_id: { required: "Please select payer" },
			payment_method: { required: "Please select payment method" },
		},
		submitHandler: function (form) {
			add_expense_formSubmit(form);
		}

	});

	// $("#xin-form").submit(function (e) {
	function add_expense_formSubmit(form) {
