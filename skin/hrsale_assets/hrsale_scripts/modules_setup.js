$(document).ready(function() {
    jQuery(".switch").change(function() {
        ...
        if ($('#m-biometric').is(':checked')) {
            var is_biometric = $("#m-biometric").val();
        } else {
            var is_biometric = '';
        }
        ...
        $.ajax({
            type: "GET",
            url: site_url + "settings/modules_info/?is_ajax=2&type=modules_info&form=2&mrecruitment=" + mrecruitment + "&mtravel=" + mtravel + "&mfiles=" + mfiles + "&mlanguage=" + mlanguage + "&morgchart=" + morgchart + "&mevents=" + mevents + "&chatbox=" + chatbox + '&is_sub_departments=' + is_sub_departments + '&module_payroll=' + module_payroll + '&module_performance=' + module_performance + '&is_biometric=' + is_biometric,
            //data: order,
