    ...
    public function promotion_list()
     {
        ...
        foreach($promotion->result() as $r) {
			$promotion_type =  $this->Xin_model->get_promotion_type_id($r->title);
			$title = $promotion_type[0]->name;	  		
			// get user > employee_
	 ...
	 public function get_promotion_type() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/promotion/get_promotion_type", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
	 ...
	 public function read() {
		...
		$data = array(
				...
				'promotion_type' => $this->Xin_model->get_all_promotion_type()
				);
			$session = $this->session->userdata('username');
		if(!empty($session)){ 
