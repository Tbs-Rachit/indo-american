...
class Payroll extends MY_Controller
{
	...
	public function payslip_list()
	{
		...
		if ($user_info[0]->user_role_id == 1 || in_array('314', $role_resources_ids)) {
			if ($this->input->get("employee_id") == 0 && $this->input->get("company_id") == 0) {
				...
				$payslip = $this->Payroll_model->get_multiple_employee_comp_template($this->input->get("company_id"), $this->input->get("employee_id"));
			} else {
				$payslip = $this->Employees_model->get_employees_payslip();
			}
		} else {
		...
		foreach ($payslip->result() as $r) {
			...
			if ($count_statutory_deductions > 0) {
				foreach ($statutory_deductions->result() as $sl_salary_statutory_deductions) {
					$deduction_name = $this->Employees_model->get_deduction_byId($sl_salary_statutory_deductions->statutory_options);
					if ($sl_salary_statutory_deductions->deduction_amount_type == 1) {
						$deduction_benefit_name = $deduction_name[0]->salary_benefit_name;
						if ($sl_salary_statutory_deductions->salary_type == 1) {
							$sta_salary = $basic_salary;
						} elseif ($sl_salary_statutory_deductions->salary_type == 2) {
							$sta_salary = $basic_salary + $allowance_amount;
						}
						$st_amount = $sl_salary_statutory_deductions->deduction_amount;
						$single_sd = $st_amount;
						
					} else if ($sl_salary_statutory_deductions->deduction_amount_type == 2) {
						$deduction_benefit_name = $deduction_name[0]->salary_benefit_name . ' (' . $sl_salary_statutory_deductions->deduction_amount . '%)';
						...
						$st_amount = $sta_salary / 100 * $sl_salary_statutory_deductions->deduction_pr_amount;
						$single_sd = $st_amount;
					}
			...
			// print_r($statutory_deductions_amount);die;
			...
			if ($result) {
				...
				if ($count_statutory_deductions > 0) {
					foreach ($salary_statutory_deductions as $sl_statutory_deduction) {
						...
						//   if($sl_statutory_deductions->salary_type == 1){
						// 	$sta_salary = $basic_salary;
						//   }elseif($sl_statutory_deductions->salary_type == 2){
						// 	$sta_salary = $basic_salary + $allowance_amount;
						//   }
						...
						//   $deduction_benefit_name = $deduction_name[0]->salary_benefit_name.' ('.$sl_statutory_deductions->deduction_amount.'%)';
						...
						if ($sl_statutory_deduction->deduction_amount_type == 1) {

							$st_amount = $sl_statutory_deduction->deduction_amount;
						} elseif ($sl_statutory_deduction->deduction_amount_type == 2) {
						...
						$statutory_deduction_data = array(
							...
							'employee_id' => $this->input->post('emp_id'),
							'salary_month' => $this->input->post('pay_date'),
							'deduction_title' => $sl_statutory_deduction->statutory_options,
							'deduction_amount' => $st_amount,
							'created_at' => date('d-m-Y h:i:s')
						);
	...
	public function pdf_create()
	{
		...
		$employee = $this->Employees_model->read_employee_information($payment[0]->employee_id);
		$tbl = '<br><br>
		...
		$dataAttr = [];
		$module_attributes = $this->Custom_fields_model->get_is_display_hrsale_module_attributes(1);
		if (!empty($module_attributes)) {
			foreach ($module_attributes as $key => $attr) {
				$attribute_info = $this->Custom_fields_model->get_employee_custom_data($user_id, $module_attributes[$key]->custom_field_id);
				if (!is_null($attribute_info)) {
					$attr_val = $attribute_info->attribute_value;
					if ($attr_val == null) {
						$attr_val = 'N/A';
					}
					$dataAttr[] = $module_attributes[$key]->attribute_label;
					$dataAttr[] = $attr_val;
				}
			}
		...
		if ($count_allowances > 0) {
			foreach ($allowances->result() as $sl_allowances) {
				$allowance_name = $this->Employees_model->get_allowance_byId($sl_allowances->allowance_title);
				$tblbrk .= '<tr>
					<td colspan="2">' . $allowance_name[0]->salary_benefit_name . '</td>
		...
		if ($count_statutory_deductions > 0) {
			foreach ($statutory_deductions->result() as $sl_statutory_deductions) {
				$deduction_name = $this->Employees_model->get_deduction_byId($sl_statutory_deductions->deduction_title);
				// if ($system[0]->statutory_fixed != 'yes') :
				// 	$sta_salary = $bs;
				// 	$st_amount = $sta_salary / 100 * $sl_statutory_deductions->deduction_amount;
				...
				$xstatutory_deduction_amount = $sl_statutory_deductions->deduction_amount;
				$tblbrk .= '<tr>
					<td colspan="2">' . $deduction_name[0]->salary_benefit_name . '</td>
					<td>&nbsp;</td>
	...
	public function get_company_payslip_location()
	{
		...
		if (is_numeric($keywords[0])) {
			...
			if (!empty($session)) {
				$data = $this->security->xss_clean($data);
				$this->load->view("admin/payroll/get_company_payslip_location", $data);
			} else {
				redirect('admin/');
			}
		}
