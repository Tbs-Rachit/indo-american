...
class Announcement extends MY_Controller
{
	...
	public function announcement_list()
	{
		...
		foreach ($announcement->result() as $r) {
			...
			if ($r->employee_id == 0) {
				$emp_ids = "All Employee";
			} else {
				$emp_id = explode(',', $r->employee_id);
				$all_employee = $this->Xin_model->get_all_employee();
				foreach ($all_employee as $employee) {
					if (in_array($employee->user_id, $emp_id)) {
	...
	public function read()
	{
		...
		$emp_id = explode(',', $result[0]->employee_id);
		$data = array(
	...
	public function get_employee()
	{
		...
		$session = $this->session->userdata('username');
		if (!empty($session)) {
			$this->load->view("admin/announcement/get_employee", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
	...
	public function add_announcement()
	{
		...
		if ($this->input->post('add_type') == 'announcement') {
			...
			$employee_id = implode(',', $this->input->post('employee_id'));
	...
	public function update()
	{
		...
		if ($this->input->post('edit_type') == 'announcement') {
			...
			$emp_id = implode(',', $this->input->post('employee_id'));
			$data = array(
				'title' => $this->input->post('title'),
				'start_date' => $this->input->post('start_date_modal'),
				'end_date' => $this->input->post('end_date_modal'),
