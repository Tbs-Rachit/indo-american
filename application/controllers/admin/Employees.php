...
class Employees extends MY_Controller
{
	...
	public function employees_list()
	{
		...
		if ($this->input->get("ihr") == 'true') {
			...
			if ($user_info[0]->user_role_id == 1 || !in_array('555', $role_resources_ids)) {
				$employee = $this->Employees_model->get_employees();
			} else {
	...
	public function detail()
	{
		...
		$data = array(
			...
			'get_company_grade' => $this->Xin_model->get_company_grade($result[0]->company_id),
			'get_all_deductions' => $this->Xin_model->get_deduction_list(),
			'get_all_allowance' => $this->Xin_model->get_allowance_list(),
	...
	public function dialog_document()
	{
		...
		$system = $this->Employees_model->all_document_types_byId($document[0]->document_type_id);
		...
		$data = array(
			...
			'all_document_types' => $this->Employees_model->all_document_types(),
			'no' => $document[0]->document_no,
			'date_of_create' => $document[0]->date_of_create,
			...
			'is_title' => $system[0]->is_title,
			'is_address' => $system[0]->is_address,
			'is_no' => $system[0]->is_no,
			'is_create_date' => $system[0]->is_create_date,
			'is_expiry_date' => $system[0]->is_expiry_date,
			'is_notif_email' => $system[0]->is_notif_email,
			'is_file' => $system[0]->is_file,
		);
		if (!empty($session)) {
			$this->load->view('admin/employees/dialog_employee_details', $data);
		} else {
	...
	public function dialog_salary_allowance()
	{
		...
		$data = array(
			...
			'allowance_amount_type' => $result[0]->allowance_amount_type,
			'allowance_pr_amount' => $result[0]->allowance_pr_amount,
	...
	public function dialog_salary_statutory_deductions()
	{
		...
		$data = array(
			...
			'employee_id' => $result[0]->employee_id,
			'deduction_salary_type' => $result[0]->salary_type,
			'deduction_amount_type' => $result[0]->deduction_amount_type,
			'deduction_pr_amount' => $result[0]->deduction_pr_amount,
	...
	public function document_info()
	{
		...
		if ($this->input->post('type') == 'document_info' && $this->input->post('data') == 'document_info') {
			...
			$notifiction =  $this->input->post('email') === '' ? $this->input->post('email') : 1;
			//clean simple fields
			...
			$date_of_create = $this->Xin_model->clean_date_post($this->input->post('date_of_create'));

			$data = array(
				'document_type_id' => $this->input->post('document_type_id'),
				'document_no' => $this->input->post('no'),
		...
		if ($this->input->post('type') == 'e_document_info') {
			...
			/* Check if file uploaded..*/ else if ($_FILES['document_file']['size'] == 0) {
				$data = array(
					'document_type_id' => $this->input->post('document_type_id'),
					'document_no' => $this->input->post('no'),
					'date_of_create' => $this->input->post('date_of_create'),
	...
	// Validate and add info in database // Tax Info

	public function add_tax_info()
	{

		if ($this->input->post('type') == 'tax_info') {
			/* Define return | here result is used to return user data and error for error message */
			$Return = array('result' => '', 'error' => '', 'csrf_hash' => '');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			...
			$data = array(
				...
				'is_aadhar_no' => $this->input->post('is_aadhar_no'),
				'is_pan_no' => $this->input->post('is_pan_no'),
				'is_un_no' => $this->input->post('is_un_no'),
			);
			$result = $this->Employees_model->tax_info_add($data, $id);
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('xin_tax_info_added');
			} else {
				$Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
	...
	public function update_allowance_info()
	{
		...
		if ($this->input->post('type') == 'e_allowance_info') {
			...
			$id = $this->input->post('user_id');
			$employee = $this->Xin_model->read_user_info($id);
			if ($this->input->post('allowance_type') == 1) {
				$pr_amount = Null;
				$amount = $this->input->post('allowance_amount');
			} else {
				$pr_amount = $this->input->post('allowance_amount');

				$amount = $employee[0]->basic_salary * $pr_amount / 100;
			}

			$data = array(
				'allowance_title' => $this->input->post('allowance_title'),
				'allowance_amount_type' => $this->input->post('allowance_type'),
			...
			$this->Employees_model->recalculate_all_ded($id, $basic_salary);
			if ($result == TRUE) {
	...
	public function update_statutory_deductions_info()
	{
		...
		if ($this->input->post('type') == 'e_salary_statutory_deductions_info') {
			...
			if ($this->input->post('ded_salary_type') == 1) {
				if ($this->input->post('ded_amount_type') == 1) {
					$pr_amount = '';
					$amount = $this->input->post('amount');
				} else {
					$pr_amount = $this->input->post('amount');
					$id = $this->input->post('user_id');
					$employee = $this->Xin_model->read_user_info($id);
					$amount = $employee[0]->basic_salary * $pr_amount / 100;
				}
			} else if ($this->input->post('ded_salary_type') == 2) {
				if ($this->input->post('ded_amount_type') == 1) {
					$pr_amount = '';
					$amount = $this->input->post('amount');
				} else {
					...
					$id = $this->input->post('user_id');
					$employee = $this->Xin_model->read_user_info($id);
					$all_amount_total = $this->Employees_model->get_allowance_total_empId($id);
					$gross_amount = $employee[0]->basic_salary + $all_amount_total[0]->allowance_amount;
					$amount = $gross_amount / 100 * $pr_amount;
				}
			}
			$data = array(
				'employee_id' => $this->input->post('user_id'),
				'statutory_options' => $this->input->post('statutory_options'),
				'salary_type' => $this->input->post('ded_salary_type'),
				'deduction_amount_type' => $this->input->post('ded_amount_type'),
					...
					if (is_uploaded_file($_FILES['file']['tmp_name'])) {
						...
						if (filesize($_FILES['file']['tmp_name']) > 2000000) {
							...
							while (($line = fgetcsv($csvFile)) !== FALSE) {
								...
								//get company ID
								$company_detail = $this->Xin_model->get_companies_on_code($line[9]);
								if ($company_detail[0]) {
								...
								if ($company_id) {
									//get Location ID
									$location_detail = $this->Department_model->ajax_company_location_information($company_id);
									$location_id = '';
									if (!empty($location_detail)) {
										foreach ($location_detail as $location) {
											if ($location->location_name == $line[10]) {
									...
									if ($location_id) {
										//get Department ID
										$department_detail = $this->Department_model->ajax_location_departments_information($location_id);
										$department_id = '';
										if (!empty($department_detail)) {
											foreach ($department_detail as $department) {
												if ($department->department_name == $line[11]) {
								...
								//get  role id by name
								$roles = $this->Roles_model->get_by_name_user_roles('Employee');
								$role_id = $roles[0]->role_id;
								//get leave type
								$leavetype = $this->Timesheet_model->all_leave_types();
								$leave_id = array();
								foreach ($leavetype as $leave) {
									$leave_id[] = $leave->leave_type_id;
								}
								$cat_ids = implode(',', $leave_id);

								if ($company_id != null && $location_id != null && $department_id != null) {
									$data = array(
										'first_name' => $line[0],
										'last_name' => $line[1],
										'username' => $line[2],
									...
									if ($this->Employees_model->check_employee_email($line[3]) > 0) {
										$Return['error'] = $this->lang->line('xin_employee_already_exist');
										$data['error'] = $this->lang->line('xin_employee_already_exist');
										$Return['exist'][] = $data;
									} else {
									...
									$Return['error'] = $error . ' ' . $this->lang->line('xin_does_not_exist');
									$data['error'] = $error . ' ' . $this->lang->line('xin_does_not_exist');
									$Return['exist'][] = $data;
								}
							}
	...
	public function update_salary_option()
	{
		...
		if ($this->input->post('type') == 'employee_update_salary') {
			...
			$check_grade = $this->Employees_model->get_grade($this->input->post('grade_id'));

			if ($this->input->post('basic_salary') === '') {
				$Return['error'] = $this->lang->line('xin_employee_salary_error_basic');
			} else if ($check_grade[0]->grade_range_from > $this->input->post('basic_salary') || $check_grade[0]->grade_range_to < $this->input->post('basic_salary')) {
				$Return['error'] = 'Please Insert Basic Salary Between ' . number_format($check_grade[0]->grade_range_from, 2) . ' to ' . number_format($check_grade[0]->grade_range_to, 2);
			}
	...
	public function employee_allowance_option()
	{
		...
		if ($this->input->post('type') == 'employee_update_allowance') {
			...
			if ($this->input->post('is_allowance_taxable') === '') {
				$Return['error'] = $this->lang->line('xin_employee_set_allowance_options_error');
			} else if ($this->input->post('salary_allowance') === '') {
				$Return['error'] = $this->lang->line('xin_employee_set_allowance_error');
			} else if ($this->input->post('allowance_type') === '') {
				$Return['error'] = $this->lang->line('xin_employee_set_amount_type_error');
			} else if ($this->input->post('allowance_amount') === '') {
				$Return['error'] = $this->lang->line('xin_employee_set_allowance_amount_error');
			}
			$get_employee_details = $this->Employees_model->get_employee_allowances($this->input->post('user_id'));
			if (!empty($get_employee_details)) {
				foreach ($get_employee_details as $get_e_all) {
					if ($get_e_all->allowance_title == $this->input->post('salary_allowance')) {
			...
			$data = array(
				'allowance_title' => $this->input->post('salary_allowance'),
				'allowance_amount_type' => $this->input->post('allowance_type'),
				'allowance_pr_amount' => $pr_amount,
				'allowance_amount' => $amount,
				'employee_id' => $this->input->post('user_id'),
			...
			$this->Employees_model->recalculate_deduction($empID, $basic_salary);
	...
	public function set_statutory_deductions()
	{
		...
		if ($this->input->post('type') == 'statutory_deductions_info') {
			...
			$Return = array('result' => '', 'error' => '', 'csrf_hash' => '');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			if ($this->input->post('statutory_options') === '') {
				$Return['error'] = $this->lang->line('xin_error_deduction_type_field');
			} else if ($this->input->post('ded_salary_type') === '') {
				$Return['error'] = $this->lang->line('xin_error_salary_type_field');
			} else if ($this->input->post('ded_amount_type') === '') {
				$Return['error'] = $this->lang->line('xin_error_amount_type_field');
			} else if ($this->input->post('amount') === '') {
				$Return['error'] = $this->lang->line('xin_error_amount_field');
			}
			$get_employee_details = $this->Employees_model->get_employee_deduction($this->input->post('user_id'));
			if (!empty($get_employee_details)) {
				foreach ($get_employee_details as $get_e_all) {
					if ($get_e_all->statutory_options == $this->input->post('statutory_options')) {
	...
	public function delete_all_allowances()
	{
		...
		if ($this->input->post('data') == 'delete_record') {
			...
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			$id = $this->uri->segment(4);
			$employee_id = $this->Employees_model->read_single_salary_allowance($id);
			$empID = $employee_id[0]->employee_id;
			$result = $this->Employees_model->delete_allowance_record($id);
			$employee = $this->Xin_model->read_user_info($empID);
			$basic_salary = $employee[0]->basic_salary;
			$this->Employees_model->recalculate_all_ded($empID, $basic_salary);
			if ($result == TRUE) {
	...
	public function salary_all_allowances()
	{
		...
		foreach ($allowances->result() as $r) {
			...
			$get_allowance = $this->Employees_model->get_allowance_byId($r->allowance_title);

			if ($r->allowance_amount_type == 1) {
				$name = $get_allowance[0]->salary_benefit_name;
			} else {
				$name = $get_allowance[0]->salary_benefit_name . ' ' . '(' . $r->allowance_pr_amount . '%)';
			}
			if ($r->allowance_salary_amount_type == 1) {
				$salary_type = $this->lang->line('xin_basic_salary');
			} elseif ($r->allowance_salary_amount_type == 2) {
				$salary_type = $this->lang->line('xin_gross_salary');
			} else {
				$salary_type = '';
			}
			if ($r->is_modify == 1) {
				$span_name = '<span style="color:green">' . $name . '</span>';
			} elseif ($r->is_modify == 0) {
				$span_name = $name;
			}
			$data[] = array(
				...
				$span_name,
				$salary_type,
				number_format($r->allowance_amount, 2)
			);
		}
	...
	public function salary_all_statutory_deductions()
	{
		...
		foreach ($statutory_deductions->result() as $r) {
			$deduction_name = $this->Employees_model->get_deduction_byId($r->statutory_options);
			if ($r->deduction_amount_type == 1) {
				$name = $deduction_name[0]->salary_benefit_name;
			} elseif ($r->deduction_amount_type == 2) {
				$name = $deduction_name[0]->salary_benefit_name . ' (' . number_format($r->deduction_pr_amount, 2) . '%)';
			}
			if ($r->salary_type == 1) {
				$salary_type = $this->lang->line('xin_basic_salary');
	...
	public function get_doc_details()
	{
		...
		if (is_numeric($keywords[0])) {
			...
			if (!empty($session)) {
				$data = $this->security->xss_clean($data);
				$this->load->view("admin/employees/get_doc_types", $data);
			} else {
				redirect('admin/setting');
			}
		}
	}
	public function get_doc_details_d()
	{
		...
		if (is_numeric($keywords[0])) {
			...
			if (!empty($session)) {
				$data = $this->security->xss_clean($data);
				$this->load->view("admin/employees/get_doc_types_d", $data);
			} else {
				redirect('admin/setting');
			}
		}
