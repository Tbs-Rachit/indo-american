...
class Timesheet extends MY_Controller
{
	...
	public function get_departments()
	{
		...
		$session = $this->session->userdata('username');
		if (!empty($session)) {
			$this->load->view("admin/timesheet/get_departments", $data);
		} else {
			redirect('admin/');
		}
					...
					if (is_uploaded_file($_FILES['file']['tmp_name'])) {
						...
						if (filesize($_FILES['file']['tmp_name']) > 512000) {
							...
							while (($line = fgetcsv($csvFile)) !== FALSE) {
								...
								$attendance_date = date('Y-m-d', strtotime($line[1]));
								$clock_in = $line[2];
								$clock_out = $line[3];
	...
	public function attendance_list()
	{
		...
		if ($user_info[0]->user_role_id == 1 || !in_array('555', $role_resources_ids)) {
			if ($this->input->get("ihr") == 'true') {
				if ($employee_id == 0) {
					$employee = $this->Xin_model->get_employees_on_department($department_id);
				} else {
					$employee = $this->Xin_model->read_employee_info_att($employee_id);
				}
			} else {
	...
	public function dtwise_attendance_list()
	{
		...
		$uid = $session['uid'];
		if ($user_role_id == 1 || !in_array('555', $role_resources_ids)) {
			$employee = $this->Xin_model->read_user_attendance_info();
		} else {
			$this->date_wise_list($uid);
		}
		$data = array();
	...
	public function date_wise_list()
	{
		...
		} else if ($user_info[0]->user_role_id == 1 || !in_array('555', $role_resources_ids)) {
			$employee_id = $this->input->get("user_id");
		} else {
			$employee_id = $session['user_id'];
		}
	...
	public function update_attendance_list()
	{
		...
		if ($employee_id != null && $employee_id == 0) {
			$attendance_employee = $this->Timesheet_model->get_all_employee_attendance_with_date($attendance_date);
		} else {
	...
	public function leave_list()
	{
		...
		foreach ($leave->result() as $r) {
			...
			if ($r->is_half_day == 1) {
				$duration = $this->Xin_model->set_date_format($r->from_date) . ' ' . $this->lang->line('dashboard_to') . ' ' . $this->Xin_model->set_date_format($r->to_date) . '<br>' . $this->lang->line('xin_hrsale_total_days') . ': ' . $r->no_of_day . ' (' . $this->lang->line('xin_hr_leave_first_half_day') . ')';
			} elseif ($r->is_half_day == 2) {
				$duration = $this->Xin_model->set_date_format($r->from_date) . ' ' . $this->lang->line('dashboard_to') . ' ' . $this->Xin_model->set_date_format($r->to_date) . '<br>' . $this->lang->line('xin_hrsale_total_days') . ': ' . $r->no_of_day . ' (' . $this->lang->line('xin_hr_leave_second_half_day') . ')';
			} else {
				$duration = $this->Xin_model->set_date_format($r->from_date) . ' ' . $this->lang->line('dashboard_to') . ' ' . $this->Xin_model->set_date_format($r->to_date) . '<br>' . $this->lang->line('xin_hrsale_total_days') . ': ' . $r->no_of_day;
			}
			...
			if ($user_info[0]->user_role_id == 1 || !in_array('555', $role_resources_ids)) {
				...
				if (in_array('288', $role_resources_ids) && $r->status == 1) { //edit
					$edit = '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_edit') . '"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light edit-data" data-toggle="modal" data-target=".edit-modal-data" data-leave_id="' . $r->leave_id . '" ><span class="fa fa-pencil"></span></button></span>';
				} else {
					$edit = '';
				}
			}
			if (in_array('289', $role_resources_ids) && $r->status == 1) { // delete
	...
	public function add_leave()
	{
		...
		if ($this->input->post('add_type') == 'leave') {
			...
			if ($this->input->post('leave_take') == 1 || $this->input->post('leave_take') == 2  && $no_of_days > 1) {
			...
			if ($this->input->post('start_date') != '') {
				...
				if ($this->input->post('leave_take') != 1 || $this->input->post('leave_take') != 2) {
			...
			if ($this->input->post('leave_take') != 1 || $this->input->post('leave_take') != 2) {
				...
				$leave_half_day_opt = $this->input->post('leave_take');
			}
			$datetime1 = new DateTime($this->input->post('start_date'));
			$datetime2 = new DateTime($this->input->post('end_date'));
			$interval = $datetime1->diff($datetime2);
			if (strtotime($this->input->post('start_date')) == strtotime($this->input->post('end_date'))) {
				if ($this->input->post('leave_type') == 1 ||  $this->input->post('leave_type') == 2) {
					$total_day = 0.5;
				} else {
					$total_day = 1;
				}
			...
			$data = array(
				...
				'status' => '1',
				'is_notify' => '1',
				'is_half_day' => $this->input->post('leave_take'),
				'created_at' => date('Y-m-d h:i:s')
			);
	...
	public function edit_leave()
	{
		...
		if ($this->input->post('edit_type') == 'leave') {
			...
			if ($this->input->post('leave_type') === '') {
				...
				$Return['error'] = $this->lang->line('xin_error_leave_type_reason');
			} else if ($this->input->post('leave_take') != 3) {
				if ($this->input->post('start_date') != $this->input->post('end_date')) {
					$Return['error'] = $this->lang->line('xin_error_leave_take');
				}
			}
