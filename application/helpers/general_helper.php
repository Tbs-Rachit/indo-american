...
if ( ! function_exists('employee_leave_halfday_cal'))
{
	function employee_leave_halfday_cal($leave_type_id,$employee_id) {
		...
		$CI->db->where('leave_type_id',$leave_type_id);
		$CI->db->where('status!=',3);
		$CI->db->where('(is_half_day=1 or is_half_day=2)',NULL);
		//$CI->db->or_where('is_half_day',1);
		$query=$CI->db->get();
...
if ( ! function_exists('get_country_from_ip_address'))
{
	function get_country_from_ip_address($result=false) {
		$ip = get_ip_address();
		if($ip != '127.0.0.1'){
			$ipdat = @json_decode(file_get_contents("http://www.geoplugin.net/json.gp?ip=" . $ip));
			if($ipdat->geoplugin_status == 200){
				$data['country_name'] = $ipdat->geoplugin_countryName;
				$data['country_code'] = $ipdat->geoplugin_countryCode;
				$data['city_name'] = $ipdat->geoplugin_city;
				$data['latitude'] = $ipdat->geoplugin_latitude;
				$data['longitude'] = $ipdat->geoplugin_longitude;
				$data['currency_symbol'] = $ipdat->geoplugin_currencySymbol;
				$data['currency_code'] = $ipdat->geoplugin_currencyCode;
				$data['timezone'] = $ipdat->geoplugin_timezone;
			}else{
				$data['country_code'] = '';
			}
			if($result){
...
if ( ! function_exists('get_user_country'))
{
	function get_user_country() {
		...
		$code = get_country_from_ip_address();
		$code = ($code != '')?$code:'IN';
		$CI->db->from('xin_countries');
		$CI->db->where('country_code',$code);
		$query=$CI->db->get();
		//echo $CI->db->last_query(); die();
		if ($query->num_rows() > 0) {
			return $query->result()[0]->country_id;
		}else{
			return '';
		}
	}
