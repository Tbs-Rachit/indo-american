...
class Timesheet_model extends CI_Model
{
	...
	public function get_all_employee_attendance_with_date($attendance_date)
	{
		$sql = 'SELECT * FROM xin_attendance_time where attendance_date = ?';
		$binds = array($attendance_date);
		$query = $this->db->query($sql, $binds);

		return $query;
	}
	public function get_leave_current_date($date)
	{
		$sql = 'SELECT * FROM xin_leave_applications';
		$query = $this->db->query($sql);
		return $query->result();
	}

	public function get_check_out_miss($date)
	{
		$sql = 'SELECT * FROM xin_attendance_time where attendance_date = ? and clock_out = ?';
		$binds = array($date, '');
		$query = $this->db->query($sql, $binds);
		return $query->result();
	}
