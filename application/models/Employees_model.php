...
class Employees_model extends CI_Model
{
	...
	public function get_employees_flt($cid)
	{
	...
	public function all_document_types_byId($id)
	{
		$query = $this->db->query("SELECT * from xin_document_type WHERE document_type_id = $id");
		return $query->result();
	}
	...
	// get employee allowances
	public function get_employee_deduction($id)
	{
	...
	public function set_employee_leave_payshlip($id)
	{
	...
	public function delete_salary_allowances_byEmpId($id)
	{
		$this->db->where('allowance_id', $id);
		$this->db->delete('xin_salary_allowances');
	}
	public function delete_salary_deduction_byEmpId($id)
	{
	...
	// Function for get dails by Id
	public function get_deduction_byId($id)
	{
		$sql = 'SELECT * FROM xin_salary_benefits WHERE salary_benefit_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function read_salary_allowance_empId($id, $type = '')
	{
		if ($type != '') {
			$sql = 'SELECT * FROM xin_salary_allowances WHERE employee_id = ? AND allowance_salary_amount_type = ?';
			$binds = array($id, $type);
		} else {
	...
	public function read_salary_deduction_empId($id, $type = '')
	{
		if ($type != '') {
			$sql = 'SELECT * FROM xin_salary_statutory_deductions WHERE employee_id = ? AND salary_type = ?';
			$binds = array($id, $type);
		} else {
			$sql = 'SELECT * FROM xin_salary_statutory_deductions WHERE employee_id = ?';
			$binds = array($id);
	...
	public function get_grade($id)
	{
		...
		$sql = 'SELECT * FROM xin_grade WHERE grade_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function update_allowance_record($id, $data)
	{
		$this->db->where('allowance_id', $id);
		if ($this->db->update('xin_salary_allowances', $data)) {
			return true;
	...
	public function update_deduction_record($id, $data)
	{
		$this->db->where('statutory_deductions_id', $id);
		if ($this->db->update('xin_salary_statutory_deductions', $data)) {
			return true;
	...
	public function get_allowance_total_empId($id, $type = '')
	{
		if ($type != '') {
			$sql = 'SELECT sum(allowance_amount) as allowance_amount FROM xin_salary_allowances WHERE employee_id = ? and allowance_salary_amount_type = ?';
			$binds = array($id, $type);
		} else {
			$sql = 'SELECT sum(allowance_amount) as allowance_amount FROM xin_salary_allowances WHERE employee_id = ?';
			$binds = array($id);
		}
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
	...
	public function get_employee_all_ById($id, $type = '')
	{
		if ($type != '') {
			$sql = 'SELECT * FROM xin_salary_allowances WHERE  grade_id = ? and allowance_salary_amount_type = ?';
			$binds = array($id, $type);
		} else {
			$sql = 'SELECT * FROM xin_salary_allowances WHERE  grade_id = ?';
			$binds = array($id);
		}
		$query = $this->db->query($sql, $binds);
	...
	public function get_grade_benefit_all_ById($id, $type = '')
	{
		if ($type != '') {
			$sql = 'SELECT * FROM xin_grade_benefit WHERE grade_benefit_type = "1" AND grade_id = ? and salary_amount_type = ?';
			$binds = array($id, $type);
		} else {
			$sql = 'SELECT * FROM xin_grade_benefit WHERE grade_benefit_type = "1" AND grade_id = ?';
			$binds = array($id);
		}
		$query = $this->db->query($sql, $binds);
	...
	// get all table rows 
	public function get_grade_benefit_ded_ById($id, $type = '')
	{
		if ($type != '') {
			$sql = 'SELECT * FROM xin_grade_benefit WHERE grade_benefit_type = "2" AND grade_id = ? and salary_amount_type = ?';
			$binds = array($id, $type);
		} else {
			$sql = 'SELECT * FROM xin_grade_benefit WHERE grade_benefit_type = "2" AND grade_id = ?';
			$binds = array($id);
		}
	...
	public function get_employees_on_grade($id)
	{
		$sql = 'SELECT * FROM xin_employees WHERE grade_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function get_employees_on_grade_benefit($id)
	{
		$sql = 'SELECT * FROM xin_grade_benefit WHERE grade_benefit_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	// Insert Seleted Grade Data in Allowance and Deduction Table

	public function insert_grade_data_1($empID, $grade_id, $basic_salary, $salary_amount_type = 1)
	{
		if ($grade_id != '') :
			...
			// Allowance Insert , Update , Delete
			$check_allowance = $this->read_salary_allowance_empId($empID); // Get Allowance Data by Employee ID

			if (!empty($check_allowance)) {
				foreach ($check_allowance as $allowance_u) :

					if ($allowance_u->grade_id != '') :
						if ($allowance_u->is_modify == 1) {
							$all_array[$allowance_u->allowance_title] = $allowance_u;
						}
						$this->delete_salary_allowances_byEmpId($allowance_u->allowance_id); // Delete Recordes

					else :
						if ($allowance_u->allowance_amount_type == 2) {

							$amount = $basic_salary * $allowance_u->allowance_pr_amount / 100;

							$data_all_u = array(
								'employee_id' => $allowance_u->employee_id,
								'grade_id' => $allowance_u->grade_id,
								'is_allowance_taxable' => $allowance_u->is_allowance_taxable,
								'allowance_title' => $allowance_u->allowance_title,
								'allowance_amount_type' => $allowance_u->allowance_amount_type,
								'allowance_pr_amount' => $allowance_u->allowance_pr_amount,
								'allowance_salary_amount_type' => $salary_amount_type,
							...
							print_r($data_all_u);
							die;
							$this->update_allowance_record($allowance_u->allowance_id, $data_all_u);
						}
					endif;
			...
			$benefit_all = $this->get_grade_benefit_all_ById($grade_id);
			if (!empty($benefit_all) && $grade_id != '') {
				foreach ($benefit_all as $allowance) :

					$modify = '0';
					if ($allowance->grade_amount_type == 1) {
						$pr_amount = null;
						$amount = $allowance->grade_amount;
					} else if ($allowance->grade_amount_type == 2) {
						$pr_amount = $allowance->grade_amount;
						$amount = $basic_salary * $pr_amount / 100;
					}
					// End  Common Calculation

					if ($allowance->grade_benefit_type == 1) : // Grade benefit type is Allowance
						$amount_type = $allowance->grade_amount_type;
						if (isset($all_array[$allowance->benefit_id])) {

							$modify = 1;
							if ($all_array[$allowance->benefit_id]->allowance_amount_type == 1) {
								$pr_amount = null;
								$amount_type = $all_array[$allowance->benefit_id]->allowance_amount_type;
								$amount = $all_array[$allowance->benefit_id]->allowance_amount;
							} else if ($all_array[$allowance->benefit_id]->allowance_amount_type == 2) {
								if ($all_array[$allowance->salary_amount_type]->allowance_salary_amount_type == 1) {
									$amount_type = $all_array[$allowance->benefit_id]->allowance_amount_type;
									$pr_amount = $all_array[$allowance->benefit_id]->allowance_pr_amount;
									$amount = $basic_salary * $pr_amount / 100;
								} else if ($all_array[$allowance->salary_amount_type]->allowance_salary_amount_type == 2) {
								}
							}
						}
						$data_all = array(
							...
							'allowance_title' => $allowance->benefit_id,
							'allowance_amount_type' => $amount_type,
							'allowance_pr_amount' => $pr_amount,
							'allowance_salary_amount_type' => $salary_amount_type,
							'allowance_amount' => $amount,
							'is_modify' => $modify,
						);
						...
						print_r($data_all);
						die;
						$this->add_salary_allowances($data_all); // Insert Allowance
					endif;
				endforeach;
			}

			// Deduction Insert , Update , Detele
			$get_deduction_u =  $this->read_salary_deduction_empId($empID); // Get Deduction Data by Employee ID

			if (!empty($get_deduction_u)) {
				foreach ($get_deduction_u as $deduction_u) :

					if ($deduction_u->grade_id != '') :
						if ($deduction_u->is_modify == 1) {
							$ded_array[$deduction_u->statutory_options] = $deduction_u;
						}
						$this->delete_salary_deduction_byEmpId($deduction_u->statutory_deductions_id); // Delete Recordes
					else :
						if ($deduction_u->deduction_amount_type == 2) {
							if ($deduction_u->salary_type == 1) {
								$amount_u = $basic_salary * $deduction_u->deduction_pr_amount / 100;
							} elseif ($deduction_u->salary_type == 2) {
								$all_amount_total = $this->get_allowance_total_empId($deduction_u->employee_id);
								$gross_salary = $basic_salary + $all_amount_total[0]->allowance_amount;
								$amount_u = $gross_salary / 100 * $deduction_u->deduction_pr_amount;
							}
							$data_ded_u = array(
								'employee_id' => $deduction_u->employee_id,
								'grade_id' => $deduction_u->grade_id,
								'statutory_options' => $deduction_u->statutory_options,
								'salary_type' => $deduction_u->salary_type,
								'deduction_amount_type' => $deduction_u->deduction_amount_type,
								'deduction_pr_amount' => $deduction_u->deduction_pr_amount,
								'deduction_amount' => $amount_u,
							);
							$this->update_deduction_record($deduction_u->statutory_deductions_id, $data_ded_u); // update Recordes
						}

					endif;
				endforeach;
			}
			$benefits_ded = $this->get_grade_benefit_ded_ById($grade_id);
			if (!empty($benefits_ded) && $grade_id != '') {
				foreach ($benefits_ded as $deduction) :

					$modify = '0';
					if ($deduction->grade_amount_type == 1) {
						$pr_amount = null;
						$amount = $deduction->grade_amount;
					} else if ($deduction->grade_amount_type == 2) {
						$pr_amount = $deduction->grade_amount;
						$amount = $basic_salary * $pr_amount / 100;
					}

					if (isset($ded_array[$deduction->benefit_id])) {
						$modify = 1;
						if ($deduction->grade_amount_type == 1) {
							$pr_amount = null;
							$amount = $ded_array[$deduction->benefit_id]->deduction_amount;
						} else if ($deduction->grade_amount_type == 2) {
							$pr_amount = $ded_array[$deduction->benefit_id]->deduction_pr_amount;
							$amount = $basic_salary * $pr_amount / 100;
						}
					}
					$data_ded = array(
						'employee_id' => $empID,
						'grade_id' => $grade_id,
						'statutory_options' => $deduction->benefit_id,
						'salary_type' => '1',
						'deduction_amount_type' => $deduction->grade_amount_type,
						'deduction_pr_amount' => $pr_amount,
						'deduction_amount' => $amount,
						'is_modify' => $modify,
					);
					$this->add_salary_deduction($data_ded); // Insert Deduction
				endforeach;
			}
		endif;
	}

	public function recalculate_all_ded($empID, $basic_salary)
	{
		$all_data = $this->read_salary_allowance_empId($empID, 2);
		if (!empty($all_data)) {
			foreach ($all_data as $all) {
				$a_amount = [];
				$grade_id = $all->grade_id;
				$benefit_all_b = $this->get_employee_all_ById($grade_id, 1); // get only BASIC SALARY recorde from grade benefit table

				$modify = '0';
				if ($all->allowance_amount_type == 1) {
					$pr_amount = null;
					$amount = $all->allowance_amount;
				} else if ($all->allowance_amount_type == 2) {
					if ($all->allowance_salary_amount_type == 1) {
						$pr_amount = $all->allowance_pr_amount;
						$amount = $basic_salary * $pr_amount / 100;
					} else {
						$total_all = $this->get_allowance_total_empId($empID, 1);
						$pr_amount = $all->allowance_pr_amount;
						$gross = $basic_salary + $total_all[0]->allowance_amount;
						$amount =  $gross * $pr_amount / 100;
					}
				}
				...
				$data_all_b = array(
					...
					'grade_id' => $grade_id,
					'is_allowance_taxable' => '0',
					'allowance_title' => $all->allowance_title,
					'allowance_amount_type' => $all->allowance_amount_type,
					'allowance_pr_amount' => $pr_amount,
					'allowance_salary_amount_type' => $all->allowance_salary_amount_type,
					'allowance_amount' => $amount,
					'is_modify' => $modify,
				);
				$this->update_allowance_record($all->allowance_id, $data_all_b); // Insert Allowance

			}
		}

		$ded_data = $this->read_salary_deduction_empId($empID, 2);
		if (!empty($ded_data)) :
			foreach ($ded_data as $deduction) :
				if ($deduction->salary_type == 1) {
					if ($deduction->deduction_amount_type == 1) {
						$amount = $deduction->deduction_amount;
					} elseif ($deduction->deduction_amount_type == 2) {
						$amount =  $basic_salary * $deduction->deduction_pr_amount / 100;
					}
				} elseif ($deduction->salary_type == 2) {
					if ($deduction->deduction_amount_type == 1) {
						$amount = $deduction->deduction_amount;
					} elseif ($deduction->deduction_amount_type == 2) {
						$all_amount_total = $this->get_allowance_total_empId($deduction->employee_id);
						$gross_salary =  $basic_salary + $all_amount_total[0]->allowance_amount;
						$amount = $gross_salary / 100 * $deduction->deduction_pr_amount;
					}
				}
				$data_ded = array(
					'employee_id' => $deduction->employee_id,
					'grade_id' => $deduction->grade_id,
					'statutory_options' => $deduction->statutory_options,
					'salary_type' => $deduction->salary_type,
					'deduction_amount_type' => $deduction->deduction_amount_type,
					'deduction_pr_amount' => $deduction->deduction_pr_amount,
					'deduction_amount' => $amount,
				);
				$this->update_deduction_record($deduction->statutory_deductions_id, $data_ded);
	...
	public function insert_grade_data($empID, $grade_id, $basic_salary, $salary_amount_type = '')
	{
		if ($grade_id != '') : // if Grade is not null or not add in any employee
			$all_array = [];
					...
					if ($allowance_u->grade_id != '') :
						...
						if ($allowance_u->allowance_amount_type == 2) {
							...
							$data_all_u = array(
								...
								'allowance_amount_type' => $allowance_u->allowance_amount_type,
								'allowance_pr_amount' => $allowance_u->allowance_pr_amount,
								'allowance_salary_amount_type' => $allowance_u->allowance_salary_amount_type,
								'allowance_amount' => $amount,
								'is_modify' => '0',
							);
							$this->update_allowance_record($allowance_u->allowance_id, $data_all_u);
			...
			// BASIC SALARY
			$benefit_all_b = $this->get_grade_benefit_all_ById($grade_id, 1); // get only BASIC SALARY recorde from grade benefit table
			if (!empty($benefit_all_b) && $grade_id != '') {
				foreach ($benefit_all_b as $allowance) :
					...
					$data_all_b = array(
						...
						'allowance_amount_type' => $amount_type,
						'allowance_pr_amount' => $pr_amount,
						'allowance_salary_amount_type' => 1,
						'allowance_amount' => $amount,
						'is_modify' => $modify,
					);

					$this->add_salary_allowances($data_all_b); // Insert Allowance
				// endif;
				endforeach;
			}

			// GROSS SALARY
			$benefit_all_g = $this->get_grade_benefit_all_ById($grade_id, 2); // get only GROSS SALARY recorde from grade benefit table
			if (!empty($benefit_all_g) && $grade_id != '') {
				foreach ($benefit_all_g as $allowance) :
					$modify = '0';
					$total_all = $this->get_allowance_total_empId($empID);
					...
					$data_all_g = array(
						...
						'allowance_amount_type' => $amount_type,
						'allowance_pr_amount' => $pr_amount,
						'allowance_salary_amount_type' => 2,
						'allowance_amount' => $amount,
						'is_modify' => $modify,
					);
					$this->add_salary_allowances($data_all_g); // Insert Allowance
				endforeach;
			}
			...
			$benefits_ded_b = $this->get_grade_benefit_ded_ById($grade_id, 1);
			if (!empty($benefits_ded_b) && $grade_id != '') {
				foreach ($benefits_ded_b as $deduction) :
					$modify = '0';
					if ($deduction->grade_amount_type == 1) {
						$pr_amount = null;
						$amount = $deduction->grade_amount;
			...
			$benefits_ded_g = $this->get_grade_benefit_ded_ById($grade_id, 2);
			if (!empty($benefits_ded_g) && $grade_id != '') {
				foreach ($benefits_ded_g as $deduction) :
					$modify = '0';
					$total_all = $this->get_allowance_total_empId($empID);
					if ($deduction->grade_amount_type == 1) {
						$pr_amount = null;
