      ...
      <div class="col-sm-12 col-xs-12 col-xl-12">
        ...
        <div class="card-block">
          <div id="accordion">
                      ...
                      <table class="datatables-demo table table-striped table-bordered dataTable no-footer">
                        <tbody>
                          ...
                          foreach ($allowances->result() as $sl_allowances) {
                            ...
                            $get_allowance = $this->Employees_model->get_allowance_byId($sl_allowances->allowance_title);
                            if ($sl_allowances->allowance_amount_type == 1) {
                              $benefit_name = $get_allowance[0]->salary_benefit_name;
                            } else if ($sl_allowances->allowance_amount_type == 2) {
                              $benefit_name = $get_allowance[0]->salary_benefit_name . ' (' . $sl_allowances->allowance_pr_amount . '%)';
                            }
                          ...
                          ?>
                            ...
                            ?>
                            <tr>
                              <td><strong><?php echo $benefit_name; ?> : </strong> <span class="pull-right"><?php echo $this->Xin_model->currency_sign($eallowance_amount); ?></span></td>
                            </tr>
                          <?php } ?>
                          <tr>
            ...
            <?php endif; ?>
            <!--------------------------------- Leave --------------------------------->
            <?php $leave = $this->Employees_model->set_employee_leave_payshlip($user_id);
            $set_leave = $leave->result();
            $e_leave_amount = 0;
            ?>
            <?php if (!empty($set_leave)) : ?>
              <div class="card hrsale-payslip mb-2">
                <div class="card-header"> <a class="text-dark collapsed" data-toggle="collapse" href="#set_leave" aria-expanded="false"> <strong><?php echo $this->lang->line('xin_hr_leave'); ?></strong> </a> </div>
                <div id="set_leave" class="collapse" data-parent="#accordion" style="">
                  <div class="box-body">
                    <div class="table-responsive" data-pattern="priority-columns">
                      <table class="datatables-demo table table-striped table-bordered dataTable no-footer">
                        <tbody>
                          <?php
                          foreach ($set_leave as $sl_leave) {
                            $get_leave_name = $this->Employees_model->get_leave_name($sl_leave->leave_type_id);
                            $leave_name = $get_leave_name->result();


                            $from_date_m =   date('m', strtotime($sl_leave->from_date));
                            $to_date_m =  date('m', strtotime($sl_leave->to_date));



                            if ($from_date_m == $to_date_m) {
                              $month_days =  cal_days_in_month(CAL_GREGORIAN, $from_date_m, 2020);
                              if ($sl_leave->is_half_day == 1 || $sl_leave->is_half_day == 2) {
                                $gross = $basic_salary + $allowance_amount;
                                $leave_amount = $gross * 0.5 / $month_days;
                              } elseif ($sl_leave->is_half_day == 3) {
                                $gross = $basic_salary + $allowance_amount;
                                $leave_amount = $gross * $sl_leave->no_of_day / $month_days;
                              }
                            } elseif ($from_date_m < $to_date_m) {
                              $month_days =  cal_days_in_month(CAL_GREGORIAN, $from_date_m, 2020);
                            }

                            $e_leave_amount += $leave_amount;

                          ?>
                            <tr>
                              <td><strong><?php echo $leave_name[0]->type_name . '<small> ( ' . $sl_leave->no_of_day . ' ) </small>' ?>:</strong> <span class="pull-right"><?php echo $this->Xin_model->currency_sign($leave_amount); ?></span></td>
                            </tr>
                          <?php } ?>
                          <tr>
                            <td><strong><?php echo $this->lang->line('xin_acc_total'); ?>:</strong> <span class="pull-right"><?php echo $this->Xin_model->currency_sign($e_leave_amount); ?></span></td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                      ...
                      <table class="datatables-demo table table-striped table-bordered dataTable no-footer">
                        <tbody>
                          ...
                          foreach ($statutory_deductions->result() as $sl_statutory_deductions) {
                            ...
                            $deduction_name = $this->Employees_model->get_deduction_byId($sl_statutory_deductions->statutory_options);
                            if ($sl_statutory_deductions->deduction_amount_type == 1) {
                              $deduction_benefit_name = $deduction_name[0]->salary_benefit_name;
                              $st_amount = $sl_statutory_deductions->deduction_amount;
                            } else if ($sl_statutory_deductions->deduction_amount_type == 2) {
                              $deduction_benefit_name = $deduction_name[0]->salary_benefit_name . ' (' . $sl_statutory_deductions->deduction_pr_amount . '%)';
                              $st_amount =  $sl_statutory_deductions->deduction_amount;
                            }
                          ...
                          ?>
                            ...
                            ?>
                            <tr>
                              <td><strong><?php echo $deduction_benefit_name; ?> : </strong> <span class="pull-right"><?php echo $this->Xin_model->currency_sign($sl_statutory_deductions->deduction_amount); ?></span></td>
                            </tr>
                          <?php } ?>
                          <tr>
            ...
            <!------------------------------ Net Salary Amount ------------------------------>
            <div class="card hrsale-payslip mb-2">
              <div class="card-header"> <a class="text-dark collapsed" data-toggle="collapse" href="#net_salary" aria-expanded="false"> <strong><?php echo $this->lang->line('xin_payroll_net_salary'); ?></strong> </a> </div>
              <div id="net_salary" class="collapse" data-parent="#accordion" style="">
                <div class="box-body">
                  <div class="table-responsive" data-pattern="priority-columns">
                    <?php

                    $total_earning = $basic_salary + $allowance_amount + $overtime_amount + $commissions_amount + $other_payments_amount;
                    $total_deduction = $loan_de_amount + $statutory_deductions_amount + $e_leave_amount;
                    ...
                    <table class="datatables-demo table table-striped table-bordered dataTable no-footer">
                      <tbody>
                        <tr>
                          <td><strong><?php echo $this->lang->line('xin_payroll_net_salary'); ?>:</strong> <span class="pull-right"><?php echo $this->Xin_model->currency_sign($total_net_salary); ?></span></td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
