...
?>
      ...
      <div class="col-md-6 selectdiv">
        <div class="form-group">
          ...
          <select class="form-control" name="leave_type" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('xin_leave_type'); ?>">
            <option value=""></option>
            <?php if ($user[0]->user_role_id == 1 || !in_array('555', $role_resources_ids)) { ?>

              <?php $employee = $this->Xin_model->read_employee_info($employee_id); ?>
              <?php $leave_categories_ids = explode(',', $employee[0]->leave_categories); ?>
              <?php foreach ($all_leave_types as $type) {
                ...
                if (in_array($type->leave_type_id, $leave_categories_ids)) { ?>
                  <?php if ($count_l == $type->days_per_year) { ?>
                    <option value="<?php echo $type->leave_type_id ?>" <?php if ($type->leave_type_id == $leave_type_id) : ?> selected <?php endif; ?>disabled> <?php echo $type->type_name; ?> (<?= $count_l; ?> / <?= $type->days_per_year ?> )</option>
                  <?php } else { ?>
                    <option value="<?php echo $type->leave_type_id ?>" <?php if ($type->leave_type_id == $leave_type_id) : ?> selected <?php endif; ?>> <?php echo $type->type_name; ?> (<?= $count_l; ?> / <?= $type->days_per_year ?> )</option>
                  <?php } ?>
              <?php }
              } ?>

            <?php } else { ?>
              <?php $leave_categories_ids = explode(',', $user[0]->leave_categories); ?>
              <?php foreach ($all_leave_types as $type) {
                $hlfcount = 0;
                //$count_l =0;
                $leave_halfday_cal = employee_leave_halfday_cal($type->leave_type_id, $employee_id);
        ...
        <div class="form-group">
          <label for="leave_take"><?php echo $this->lang->line('xin_hr_leave_take'); ?></label>
          <select class="form-control" name="leave_take" id="leave_take" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('xin_hr_leave_take'); ?>">
            <option value=""></option>
            <option value="1" <?= $is_half_day == 1 ? 'selected' : ' '; ?>><?php echo $this->lang->line('xin_hr_leave_first_half_day'); ?></option>
            <option value="2" <?= $is_half_day == 2 ? 'selected' : ' '; ?>><?php echo $this->lang->line('xin_hr_leave_second_half_day'); ?></option>
            <option value="3" <?= $is_half_day == 3 ? 'selected' : ' '; ?>><?php echo $this->lang->line('xin_hr_leave_full_day'); ?></option>
          </select>
        </div>
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input class="form-control e_date" placeholder="<?php echo $this->lang->line('xin_start_date'); ?>" readonly="true" name="start_date" id="s_date" type="text" value="<?php echo $from_date; ?>">
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="end_date"><?php echo $this->lang->line('xin_end_date'); ?></label>
              <input class="form-control e_date" placeholder="<?php echo $this->lang->line('xin_end_date'); ?>" readonly="true" name="end_date" id="e_date" type="text" value="<?php echo $to_date; ?>">
            </div>
          </div>
        </div>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      jQuery('#s_date').on('change', function() {
        var date = $(this).val();
        $("#e_date").datepicker("option", "minDate", date)
      });
      ...
      $("#edit_leave").validate({
        ...
        messages: {
          ...
          leave_take: {
            required: "Please select leave take"
          },
          start_date: {
        ...
        submitHandler: function(form) {
          edit_leaveformSubmit(form);
        }
      });

      // $("#edit_leave").submit(function(e) {
      function edit_leaveformSubmit(form) {
        /*Form Submit*/
        // e.preventDefault();
        var obj = $(form),
    ...
    <div class="modal-body">
      <table class="footable-details table table-striped table-hover toggle-circle">
        <tbody>
          ...
          <tr>
            <th><?php echo $this->lang->line('xin_hr_leave_take'); ?></th>
            <td style="display: table-cell;"><?php echo $leave_take; ?></td>
          </tr>
          <tr>
          ...
          <?php if ($status == '4') : ?> <?php $status_lv = $this->lang->line('xin_role_first_level_approved'); ?> <?php endif; ?>
          <tr>
