...
<div class="modal-body">
  ...
  <div class="row">
    ...
    <div class="col-md-3">  
        <div class="form-group">
          <label for="xin_faxn"><?php echo $this->lang->line('xin_hrsale_field_display');?></label>
          <select class="form-control" id="is_display" name="is_display" data-plugin="select_hrm">
          <option value="0" <?= ($is_display == 0) ? "selected" : ""; ?> >No</option>
          <option value="1" <?= ($is_display == 1) ? "selected" : ""; ?>>Yes</option>
          </select>
        </div>
    </div>
  </div>
...
<script type="text/javascript">
 $(document).ready(function(){
    ...
    /* Edit data */
    $("#edit_custom_field").validate({
      rules: {
        module_id: { required: true },
        attribute: { required: true },
        attribute_label: { required: true },
      ...
      messages: {
        ...
        attribute: { required: "Please enter attribute name" },
        attribute_label: { required: "Please enter attribute lable" },
        priority: { required: "Please enter priority" },
        validation: { required: "Please select validation" },
        attribute_type: { required: "Please select attribute type" },
      },
      submitHandler: function (form) {
        edit_custom_fieldformSubmit(form);
      }
    });
		// $("#edit_custom_field").submit(function(e){
		function edit_custom_fieldformSubmit(form){
		// e.preventDefault();
